...
var LustreServerComponent = class extends HTMLElement {
    ...
    for (const attr of attrs2) {
      ...
      Object.defineProperty(this, attr, {
        ...
        set(value) {
          ...
          if (isEqual(prev2, value)) return;
          this[`__mirrored__${attr}`] = value;
          this.#socket?.send(JSON.stringify([attrs, [[attr, value]]]));
        },
      });
    }
  ...
  #diff([diff2]) {
    const prev =
      this.shadowRoot.childNodes[this.#adoptedStyleElements.length - 1] ??
      this.shadowRoot.appendChild(document.createTextNode(""));
    const dispatch = (handler) => (event2) => {
      const msg = handler(event2);
      this.#socket?.send(JSON.stringify([event, msg.tag, msg.data]));
